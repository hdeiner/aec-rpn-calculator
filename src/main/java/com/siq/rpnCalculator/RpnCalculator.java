...
public class RpnCalculator {
    ...
    public Token calculate() throws Exception {
        while (tokenizer.hasMoreTokens()) {
            ...
            if (token.isMonadic()) {
                handleMonadicOperator(token);
            }
            if (token.isDyadic()) {
                handleDyadicOperator(token);
            }
            if (token.isStackOperator()) {
                handleStackOperator(token);
            }
        }
        if (calculatorStack.size() > 1) {
            throw new Exception("unused tokens on calculator stack");
        }
    ...
    private void handleMonadicOperator(Token token) throws Exception {
        ...
        if (calculatorStack.empty()) {
            throw new Exception("Monadic operator of \"" + token.toString() + "\" encountered, but there is no operand on the stack");
        }
    ...
    private void handleDyadicOperator(Token token) throws Exception {
        calculatorStack.pop();
        if (calculatorStack.empty()) {
            throw new Exception("Dyadic operator of \"" + token.toString() + "\" encountered, but there is no operand on the stack");
        }
        Token operand1 = calculatorStack.pop();
        if (calculatorStack.empty()) {
            throw new Exception("Dyadic operator of \"" + token.toString() + "\" encountered, but there is only one operand on the stack");
        }
    ...
    private void handleStackOperator(Token token) throws Exception {
        ...
        if (token.getOperatorType() == Token.OPERATOR.SWAP) {
            if (calculatorStack.size() < 2) {
                throw new Exception("can't SWAP with less than two items on the calculator stack");
            }
            Token operand1 = calculatorStack.pop();
            Token operand2 = calculatorStack.pop();
        ...
        if (token.getOperatorType() == Token.OPERATOR.DROP) {
            if (calculatorStack.size() == 0) {
                throw new Exception("can't DROP with an empty calculator stack");
            }
            calculatorStack.pop();
        }
        if (token.getOperatorType() == Token.OPERATOR.CLEAR) {
            calculatorStack.clear();
        }
        if (token.getOperatorType() == Token.OPERATOR.ROLL) {
            if (calculatorStack.size() == 0) {
                throw new Exception("can't ROLL with an empty calculator stack");
            }
    ...
    private Token dyadicOperator(Token operator, Token operand1, Token operand2){
        ...
        if (operator.getOperatorType() == Token.OPERATOR.PLUS) {
            result = new Token(Double.toString(arg2 + arg1));
        }
        if (operator.getOperatorType() == Token.OPERATOR.MINUS) {
            result = new Token(Double.toString(arg2 - arg1));
        }
        if (operator.getOperatorType() == Token.OPERATOR.MULTIPLY) {
            result = new Token(Double.toString(arg2 * arg1));
        }
        if (operator.getOperatorType() == Token.OPERATOR.DIVIDE) {
            result = new Token(Double.toString(arg2 / arg1));
        }
        return result;
    }
    ...
    private Token monadicOperator(Token operator, Token operand1){
        Token result = new Token("");
        Double arg1 = Double.parseDouble(operand1.toString());
        if (operator.getOperatorType() == Token.OPERATOR.PERCENT) {
            result = new Token(Double.toString(arg1 * 0.01));
        }
        return result;
    }
